...
#pragma warning restore 0414
        ...
        /// </summary>
        private static SecurityList<IMessage> mMessages = new SecurityList<IMessage>();
        ...
        /// </summary>
        private static SecurityDictionary<int, SecurityDictionary<int, MessageHandler>> mMessageHandlers = new SecurityDictionary<int, SecurityDictionary<int, MessageHandler>>();

        /// <summary>
        /// Queues listeners to be added after processing of the current 
        /// list of listeners has finished
        /// </summary>
        private static SecurityList<MessageListenerDefinition> mListenerAdds = new SecurityList<MessageListenerDefinition>();

        /// <summary>
        /// Queues listeners to be removed after the processing of the current
        /// list of listeners has finished
        /// </summary>
        private static SecurityList<MessageListenerDefinition> mListenerRemoves = new SecurityList<MessageListenerDefinition>();

        private static SecurityQueue<SafeWorkItemClass> mWorkQueue = new SecurityQueue<SafeWorkItemClass>();
        // private static SecurityQueue<Action> mWorkQueue = new SecurityQueue<Action>();
        private class SafeWorkItemClass
        {
            public IMessage message;

            public MessageHandler Handler;

            public SafeWorkItemClass(IMessage _message, MessageHandler _Handler)
            {
            ...
            public void Run()
            {
                ...
                {
                    message.IsSent = true;
                   // util.Log.log("Run执行" + message.Type + "^" + message.Recipient);
                    Handler(message);
        ...
        {
            foreach (int lType in mMessageHandlers.Keys)
            {
        ...
        public static void AddListener(int rMessageType, MessageHandler rHandler)
        {
            AddListener(rMessageType, Message.FilterTypeNothing, rHandler, false);
        }
        ...
        public static void AddListener(int rMessageType, MessageHandler rHandler, bool rImmediate)
        {
            AddListener(rMessageType, Message.FilterTypeNothing, rHandler, rImmediate);
        }
        ...
        /// <param name="rMessageType">Message we want to listen for</param>
        /// <param name="rHandler">Hander to handle the message</param>
        //public static void AddListener(UnityEngine.Object rOwner, int ? rMessageType, MessageHandler rHandler)
        //{
        ...
        /// <param name="rHandler">Hander to handle the message</param>
        /// <param name="rImmediate">Determines if the function ignores the cache and forces the listener into the list now</param>
        //public static void AddListener(UnityEngine.Object rOwner, int? rMessageType, MessageHandler rHandler, bool rImmediate)
        //{
        //    // If there is no owner, setup a general listener
        //    if (rOwner == null)
        //    {
        //        AddListener(rMessageType, null, rHandler, rImmediate);
        //    }
        ...
        /// <param name="rHandler">Hander to handle the message</param>
        public static void AddListener(int  rMessageType, int  rFilter, MessageHandler rHandler)
        {
        ...
        /// <param name="rHandler">Hander to handle the message</param>
        /// <param name="rImmediate">Determines if the function ignores the cache and forces the listener into the list now</param>
        public static void AddListener(int  rMessageType, int  rFilter, MessageHandler rHandler, bool rImmediate)
        {
           // util.Log.log("AddListener>>rMessageType" + rMessageType + " rFilter>" + rFilter + " MessageHandler" + rHandler);
        ...
        /// <param name="rMessageType">Message we want to listen for</param>
        /// <param name="rHandler">Hander to handle the message</param>
        public static void RemoveListener(int rMessageType, MessageHandler rHandler)
        {
            RemoveListener(rMessageType, Message.FilterTypeNothing, rHandler, false);
        }
        ...
        public static void RemoveListener(int rMessageType, MessageHandler rHandler, bool rImmediate)
        {
            RemoveListener(rMessageType, Message.FilterTypeNothing, rHandler, rImmediate);
        }
        ...
        /// <param name="rHandler">Hander to handle the message</param>
        public static void RemoveListener(int rMessageType, int rFilter, MessageHandler rHandler)
        {
        ...
        /// <param name="rHandler">Hander to handle the message</param>
        /// <param name="rImmediate">Determines if the function ignores the cache and forcibly removes the listener from the list now</param>
        public static void RemoveListener(int rMessageType, int rFilter, MessageHandler rHandler, bool rImmediate)
        {
        ...
        public static void SendMessage(int rType)
        {
            // Create the message
            Message lMessage = Message.Allocate();
            lMessage.Sender = null;
            lMessage.Recipient = Message.FilterTypeNothing;
            lMessage.Type = rType;
            lMessage.Data = null;
        ...
        public static void SendMessage(int rType, int  rFilter)
        {
            // Create the message
            Message lMessage = Message.Allocate();
            lMessage.Sender = null;
        ...
        public static void SendMessage(int rType, float rDelay)
        {
            // Create the message
            Message lMessage = Message.Allocate();
            lMessage.Sender = null;
        ...
        /// <param name="rFilter">Filter to send only to those listening to the filter</param>
        /// <param name="rDelay">Delay (in seconds) before sending</param>
        public static void SendMessage(int rType, int rFilter, float rDelay)
        {
            // Create the message
            Message lMessage = Message.Allocate();
            lMessage.Sender = null;
        ...
        public static void SendMessage(object rSender, int rType, object rData, float rDelay)
        {
            // Create the message
            Message lMessage = Message.Allocate();
        ...
        /// <param name="rData">Data to send</param>
        /// <param name="rDelay">Seconds to delay</param>
        //public static void SendMessage(object rSender, int  rRecipient, int rType, object rData, float rDelay)
        //{
        //    // Create the message
        //    Message lMessage = Message.Allocate();
        //    lMessage.Sender = rSender;
        ...
        public static void SendMessage(object rSender, int rRecipient, int rType, object rData, float rDelay)
        {
            //if (rType != Scripts.JSBindingClasses.Message.MessagePool.OnApplicationUpdate && rType != Scripts.JSBindingClasses.Message.MessagePool.OnApplicationLastUpdate && rType != Scripts.JSBindingClasses.Message.MessagePool.OnApplicationFixupdate)
           // util.Log.log("SendMessage>messageType>" + rType + " FilterType>" + rRecipient + (rSender == null ? " rSender>null" : ("  rSender>" + rSender.ToString()))+(rData == null ? " rData>null" : (" rData>" + rData.ToString())));
            //// Create the message
            Message lMessage = Message.Allocate();
            lMessage.Sender = rSender;
            lMessage.Recipient = rRecipient;
        ...
        public static void Update()
        {
            ...
            for (int i = 0; i < mWorkQueue.Count; i++)
            {
                SafeWorkItemClass SafeWorkItem;
                mWorkQueue.Dequeue(out SafeWorkItem);
                i--;
                if (SafeWorkItem != null)
                {
                  TaskManager.CreateTask(CoroutineFactory.GenerateCoroutine(new Action(()=>
                    {
                        SafeWorkItem.Run();
                    })
                ), (@bool) => { }).Start();
