...
namespace GameBase
{
    ...
    public class Request
    {
        ...
        public Request(string method, string uri, string data)
        {
            this.method = method;
            this.uri = new Uri( uri );
            this.bytes = Encoding.UTF8.GetBytes(data);
        ...
        private void GetResponse() {
            ...
            {
//#if !UNITY_EDITOR
                //UnityEngine.Debug.Log("NET: " + InfoString( VerboseLogging ));
//#else
...
//#endif
        ...
        void WriteToStream( Stream outputStream )
        {
            ...
            Debugger.LogError("uri.PathAndQuery->" + uri.PathAndQuery + "^" + protocol);
            ...
            foreach (string name in headers.Keys)
            {
                foreach (string value in headers[name])
                {
                    Debugger.Log("http header->" + name + "^" + value);
            ...
            {
             //   Debug.LogError(">>>>>>>>>0");
                Debugger.LogError("http send ->" + System.Text.Encoding.UTF8.GetString(bytes)) ;
                stream.Write( bytes );
            }
        }
